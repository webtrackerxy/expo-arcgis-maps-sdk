...
The example application is published on both app stores, with one screen per capability, so you can
explore the SDK on a device without setting up a development environment:

- iOS: [ArcGIS SDK for Expo on the App Store](https://apps.apple.com/gb/app/arcgis-sdk-for-expo/id6804108944)
- Android: [ArcGIS SDK for Expo on Google Play](https://play.google.com/store/apps/details?id=uk.co.sincetech.expo.arcgis.maps.sdk)
